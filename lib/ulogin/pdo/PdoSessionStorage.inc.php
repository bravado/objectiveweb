...
class ulPdoSessionStorage
{
	...
	private function Lock($id)
	{
		...
		$stmt = ulPdoDb::Prepare(
			'session',
			"INSERT INTO ".UL_TABLE_PREFIX."sessions (id, data, session_expires, lock_expires) VALUES (?, '', ?, ?)"
		);
		if (!ulPdoDb::BindExec(
			$stmt,
			NULL,		// output
		...
		))
		{
			if (ulPdoDb::ErrorCode() == '23000')
			{
				...
				while(!isset($this->lock_acquired[$id]))
				{
					...
					$session_expires = ulUtils::date_seconds_add(new DateTime(), $this->lifetime)->format(UL_DATETIME_FORMAT);
					$lock_expires = ulUtils::date_seconds_add(new DateTime(), $this->max_execution_time)->format(UL_DATETIME_FORMAT);
					$stmt = ulPdoDb::Prepare('session', 'UPDATE '.UL_TABLE_PREFIX.'sessions SET session_expires=?, lock_expires=? WHERE id=? AND lock_expires<?');
					if (!ulPdoDb::BindExec(
						$stmt,
						NULL,		// output
						array(		// input
	...
	private function Unlock($id)
	{
		...
		$stmt = ulPdoDb::Prepare('session', 'UPDATE '.UL_TABLE_PREFIX.'sessions SET lock_expires=? WHERE id=?');
		if (!ulPdoDb::BindExec(
			$stmt,
			NULL,		// output
			array(		// input
	...
	public function read($id)
	{
		...
		// Read database
		$now = ulUtils::nowstring();
		$stmt = ulPdoDb::Prepare('session', 'SELECT data FROM '.UL_TABLE_PREFIX.'sessions WHERE id=? AND session_expires>?');
		if (!ulPdoDb::BindExec(
			$stmt,
			array(		// output
				&$data, 'lob'
	...
	public function write($id, $data)
	{
		...
		$stmt = ulPdoDb::Prepare(
			'session',
			'UPDATE '.UL_TABLE_PREFIX.'sessions SET data=? WHERE id=?'
		);
		if (!ulPdoDb::BindExec(
			$stmt,
			NULL,		// output
	...
	{
		$stmt = ulPdoDb::Prepare('session', 'DELETE FROM '.UL_TABLE_PREFIX.'sessions WHERE id=?');
	...
	public function gc()
	{
		...
		// Delete old sessions
		$stmt = ulPdoDb::Prepare('session', 'DELETE FROM '.UL_TABLE_PREFIX.'sessions WHERE session_expires<=?');
		ulPdoDb::BindExec(
			$stmt,
			NULL,		// output
			array(		// input
