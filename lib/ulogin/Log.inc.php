...
class ulLog
{
  ...
  public static function ShowDebugConsole()
  {
	...
	public static function Log($action, $user, $ip, $comment='')
	{
		...
		$stmt = ulPdoDb::Prepare('log', 'INSERT INTO '.UL_TABLE_PREFIX.'log (timestamp, action, comment, user, ip) VALUES (?, ?, ?, ?, ?)');
	...
	public static function Clean()
	{
		...
		if (UL_MAX_LOG_AGE > 0)
		{
			...
			$stmt = ulPdoDb::Prepare('log', 'DELETE FROM '.UL_TABLE_PREFIX.'log WHERE timestamp<?');
			if (!ulPdoDb::BindExec(
				$stmt,
				NULL,		// output
				array(		// input
		...
		{
			$log_num_rows = 0;
			$stmt = ulPdoDb::Prepare('log', 'SELECT COUNT(*) FROM '.UL_TABLE_PREFIX.'log');
			if (!ulPdoDb::BindExec(
				$stmt,
				array(		// output
					&$log_num_rows, 'int'
			...
			if ($log_num_rows > UL_MAX_LOG_RECORDS)
			{
				...
				$stmt = ulPdoDb::Prepare('log', 'DELETE FROM '.UL_TABLE_PREFIX.'log ORDER BY timestamp ASC LIMIT ?');
				if (!ulPdoDb::BindExec(
					$stmt,
					NULL,		// output
					array(		// input
	...
	public static function GetFrequencyForUser($username, $action, $window)
	{
		...
		$stmt = ulPdoDb::Prepare('log', 'SELECT COUNT(*) FROM '.UL_TABLE_PREFIX.'log WHERE action=? AND timestamp>? AND user=?');
		if (!ulPdoDb::BindExec(
			$stmt,
			array(		// output
	...
	public static function GetFrequencyForIp($ip, $action, $window)
	{
		...
		$ip_login_attempts = 0;
		$time_before_window = ulUtils::date_seconds_sub(new DateTime(), $window)->format(UL_DATETIME_FORMAT);
		$stmt = ulPdoDb::Prepare('log', 'SELECT COUNT(*) FROM '.UL_TABLE_PREFIX.'log WHERE action=? AND timestamp>? AND ip=?');
		if (!ulPdoDb::BindExec(
			$stmt,
			array(		// output
	...
	public static function GetUserLastLoginAgo($username)
	{
		...
		// Get the number of login attempts to an account
		$last_login = '';
		$stmt = ulPdoDb::Prepare('log', "SELECT timestamp FROM ".UL_TABLE_PREFIX."log WHERE user=? AND action='auth-success' ORDER BY timestamp DESC LIMIT 1");
		if (!ulPdoDb::BindExec(
			$stmt,
			array(		// output
				&$last_login, 'str'
	...
	public static function GetIpLastLoginAgo($ip)
	{
		...
		// Get the number of login attempts to an account
		$last_login = '';
		$stmt = ulPdoDb::Prepare('log', "SELECT timestamp FROM ".UL_TABLE_PREFIX."log WHERE ip=? AND action='auth-success' ORDER BY timestamp DESC LIMIT 1");
		if (!ulPdoDb::BindExec(
			$stmt,
			array(		// output
				&$last_login, 'str'
