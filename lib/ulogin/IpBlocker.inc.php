...
class ulIpBlocker
{
	...
	public static function SetBlock($ip, $block)
	{
		...
		if ($block > 0)
		{
			...
			$stmt = ulPdoDb::Prepare('log', 'INSERT INTO '.UL_TABLE_PREFIX.'blocked_ips (ip, block_expires) VALUES (?, ?)');
			...
			if (!$query_ret && (ulPdoDb::ErrorCode() == '23000'))
			{
				...
				$stmt = ulPdoDb::Prepare('log', 'UPDATE '.UL_TABLE_PREFIX.'blocked_ips SET block_expires=? WHERE ip=?');
				$query_ret = ulPdoDb::BindExec(
					$stmt,
					NULL,		// output
					array(		// input
		...
		{
			$stmt = ulPdoDb::Prepare('log', 'DELETE FROM '.UL_TABLE_PREFIX.'blocked_ips WHERE ip=?');
			$query_ret = ulPdoDb::BindExec(
				$stmt,
				NULL,		// output
				array(		// input
	...
	public static function IpBlocked($ip)
	{
		...
		$stmt = ulPdoDb::Prepare('log', 'SELECT block_expires FROM '.UL_TABLE_PREFIX.'blocked_ips WHERE ip=?');
		if (!ulPdoDb::BindExec(
			$stmt,
			array(		// output
				&$block_expires, 'str'
